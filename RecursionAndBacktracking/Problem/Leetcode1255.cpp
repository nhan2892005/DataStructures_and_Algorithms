...
int Solution::dfs(  const vector<string>& words, 
...
int Solution::useWord(  const vector<string>& words, 
...
void Solution::unuseWord(const vector<string>& words, int i, vector<int>& count) {
    for (const char c : words[i])
        ++count[c - 'a'];
}

int Solution::maxScoreWords(vector<string>& words, 
                            vector<char>& letters,
                            vector<int>& score) {
    vector<int> count(26);
